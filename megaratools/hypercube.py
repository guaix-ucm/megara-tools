#
# Copyright 2017-2024 Universidad Complutense de Madrid
#
...
def helio_corr(ifile):
    ...
    roque = EarthLocation.from_geodetic(
        lat=28.7606 * u.deg, lon=342.1184 * u.deg, height=2326 * u.m
    )
    sc = SkyCoord(ra=h1_0["RADEG"] * u.deg, dec=h1_0["DECDEG"] * u.deg)
    heliocorr = sc.radial_velocity_correction(
        "heliocentric", obstime=Time(obs_date), location=roque
    )
...
def hypercube_dimensions(list_cubes, xoff, yoff, target_scale):
    ...
    pixel_size_x = target_scale / np.cos(
        hdu[0].header["DECDEG"] * np.pi / 180
    )  # Este es el que aparece al medir en la imagen # arcsec
    ...
    hypercube_size_x = int(
        round((abs(max(RA_list) - min(RA_list)) * 3600 / pixel_size_x) + data_size_x)
    )  # De centro a centro + otro apuntado entero
    hypercube_size_y = int(
        round((abs(max(DEC_list) - min(DEC_list)) * 3600 / pixel_size_y) + data_size_y)
    )  # De centro a centro + otro apuntado entero
...
def mask_bin(list_cubes, outfile, xoff, yoff, target_scale):
    (
        ...
        data_size_x,
        data_size_y,
    ) = hypercube_dimensions(list_cubes, xoff, yoff, target_scale)
    hdu = fits.open(list_cubes[0])
    mask = np.zeros((hypercube_size_y, hypercube_size_x), int)
    for i, ifile in enumerate(list_cubes):
        offset_pixels_x = int(
            round((abs((RA_list[i] - max(RA_list)) * 3600 / pixel_size_x)))
        )
        offset_pixels_y = int(
            round((abs((DEC_list[i] - min(DEC_list)) * 3600 / pixel_size_y)))
        )
...
def grid_combined_cube(
    list_cubes, helio_corr_apply, outfile, xoff, yoff, scale_a, scale_m, target_scale
):
    (
        hypercube_size_x,
        hypercube_size_y,
    ...
    alldata = np.zeros(
        (hypercube_size_z, hypercube_size_y, hypercube_size_x, len(list_cubes)), float
    )
    ...
    if helio_corr_apply:
        ...
        for i, ifile in enumerate(list_cubes):
            ...
            spec_helio_corr = np.zeros(
                (len(lambda_completa), data_ifile.shape[1], data_ifile.shape[2])
            )
            for j in range(data_ifile.shape[1]):
                for k in range(data_ifile.shape[2]):
                    spec_helio_corr[:, j, k] = rebin_spec(
                        lambda_em, data_ifile[:, j, k], lambda_completa
                    )
                    alldata[:, j + offset_pixels_y, k + offset_pixels_x, i] = (
                        spec_helio_corr[:, j, k] * scale_m[i] + scale_a[i]
                    )
        ...
        for i, ifile in enumerate(list_cubes):
            ...
            for j in range(data_size_y):
                for k in range(data_size_x):
                    alldata[:, j + offset_pixels_y, k + offset_pixels_x, i] = (
                        data_ifile[:, j, k] * scale_m[i] + scale_a[i]
                    )
    ...
    while while_loop_count <= (len(list_cubes) - 1):
        for i in range(len(list_cubes) - 1):
            if np.isin(i + 1, flux_calibrated_images_index) is False:
                npixels_solape = 0
                indice_solape_0 = []
                indice_solape_1 = []
                for m, valm in enumerate(flux_calibrated_images_index):
                    ...
                    if npixels_solape >= 10:
                        ...
                        flux_factor = sum(
                            np.sum(
                                alldata[
                                    400:3900, indice_solape_0, indice_solape_1, valm
                                ],
                                axis=1,
                            )
                        ) / sum(
                            np.sum(
                                alldata[
                                    400:3900, indice_solape_0, indice_solape_1, i + 1
                                ],
                                axis=1,
                            )
                        )
...
def main(args=None):
    ...
    parser.add_argument(
        "rss", help="RSS file / List of RSS files", type=argparse.FileType("rb")
    )
    parser.add_argument(
        "-l",
        "--is-a-list",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Use for -s being a list of cubes (not rss) spectra",
    )
    parser.add_argument(
        "-p",
        "--pixel-size",
    ...
    parser.add_argument(
        "-d", "--disable-scaling", action="store_true", help="Disable flux conservation"
    )
    parser.add_argument(
        "-m",
        "--method",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Use for trimming the cubes",
    )
    parser.add_argument(
        "-hyp",
        "--hyper",
        default=False,
        action="store_true",
        help="Use for creating the hypercube",
    )
    parser.add_argument(
        "-helio",
        "--helio",
        default=False,
        action="store_true",
        help="Use for applying heliocentric velocity correction",
    )
    parser.add_argument(
        "-trimn",
        "--trimming-numbers",
        nargs="*",
        default=[1, 2, 1, 1],
        help="Use for declare the number of rows and columns you want to trim. "
             "[Bottom rows, top rows, left column, right column] (default= 1,2,1,1)",
    )
    ...
    if args.is_a_list:
        xoff, yoff, scale_a, scale_m = np.loadtxt(
            args.rss, usecols=(1, 2, 3, 4), unpack=True
        )
        args.rss.seek(0)
    ...
    if not args.is_a_cube:
        ...
        for i in list_files:
            ...
            cube = mcube.create_cube_from_rss(
                rss, hexspline_order, target_scale, conserve_flux=conserve_flux
            )
