...
# Parser
    ...
    for i in range(rings_number):
        ...
        if args.save_plots:
            plot_ring(b, ell_x_offset, ell_y_offset, 90 + args.position_angle, amp_ext[i], amp_int[i], args.ellipticity)
...
def plot_ring(b, ell_x_offset, ell_y_offset, angle, a_ext, a_int, ellipticity):
    ...
    for ind, val in enumerate(b):
        ...
        hex_patch = Polygon(vertices, closed=True, fill=True, edgecolor='w', facecolor='b', alpha=0.2)
    ...
    if not ring_area.is_empty:
        ring_patch = PathPatch(Path.make_compound_path(Path(ring_area.exterior.coords), *[Path(interior.coords) for interior in ring_area.interiors]), facecolor='r', edgecolor='k', alpha=0.5)
        ax.add_patch(ring_patch)

    # Dibujar la elipse interior con transparencia
    if not ellipse_int.is_empty:
        int_patch = PathPatch(Path(ellipse_int.exterior.coords), facecolor='none', edgecolor='g', alpha=0.5)
