...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Cleaning of diffuse light from a reduced (non-RSS) MEGARA image",
        prog="clean_diffuse_light",
    )
    parser.add_argument(
        "-i",
        "--input",
        metavar="INPUT-IMAGE",
        help="Reduced FITS image",
    ...
    parser.add_argument(
        ...
        default="model.fits",
        help="Output diffuse-light FITS image",
        type=argparse.FileType("wb"),
    )
    parser.add_argument(
        "-r",
        "--residuals",
        metavar="RESIDUALS-IMAGE",
        default="residuals.fits",
        help="Output residual FITS image",
        type=argparse.FileType("wb"),
    )
    parser.add_argument(
        "-t",
        "--traces",
        metavar="MASTER-TRACES",
        help="Master traces JSON file",
        type=argparse.FileType("r"),
    )
    parser.add_argument(
        "-s",
        "--shift",
        metavar="SHIFT-TRACES",
        default=0.0,
        help="Traces shift",
        type=float,
    )
    parser.add_argument(
        "-w",
        "--window",
        metavar="SEARCH-WINDOW",
        default=6.0,
        help="Window around traces to search for non-illuminated fibers",
        type=float,
    )
    parser.add_argument(
        "-d",
        "--degree",
        metavar="DEGREE-POLY-COLS",
        default=6.0,
        help="Degree of polynomial fit for columns",
        type=float,
    )
    parser.add_argument(
        "-d2",
        "--degree-rows",
        metavar="DEGREE-POLY-ROWS",
        default=6.0,
        help="Degree of polynomial fit for rows",
        type=float,
    )
    parser.add_argument(
        "-p",
    ...
    parser.add_argument(
        "-b",
        "--binning",
        metavar="SPECTRAL-BINNING",
        default=50,
        help="Binning in the spectral direction",
        type=int,
    )
    parser.add_argument(
        "-e",
        "--exclude",
        metavar="EXCLUDE-REGION",
        help="Exclude region (c1 c2 r1 r2), e.g. 2407 2720 0 164",
        nargs="+",
        type=int,
    )
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Two-dimensional fitting?",
    )
    parser.add_argument(
        "-E",
        "--edge",
        metavar="EXCLUDE EDGE",
        default=3,
        help="Exclude this number of pixels from each edge",
        type=int,
    )
