...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Input spectrum and table", prog="plot_spectrum"
    )
    parser.add_argument(
        "-s",
        "--spectrum",
    ...
    parser.add_argument(
        ...
        "--std-table",
        metavar="STD-TABLE",
        help="Standard-star spectrum table",
    ...
    parser.add_argument(
        ...
        "--redshift",
        metavar="LINECAT-Z",
        help="Redshift for catalog lines",
        type=float,
    )
    parser.add_argument(
        "-o",
    ...
    parser.add_argument(
        "-e", "--efficiency", default=False, action="store_true", help="Efficiency?"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-L1",
        "--min-lambda",
        metavar="INITIAL LAMBDA",
        help="Initial (rest-frame) lambda to plot",
        type=float,
    )
    parser.add_argument(
        "-L2",
        "--max-lambda",
        metavar="LAST LAMBDA",
        help="Last (rest-frame) lambda to plot",
        type=float,
    )
    parser.add_argument(
        "-F1",
    ...
    gtceffarea = 7.3e5  # GTC effective area in cm**2
    # Internal parameters
    ...
    # Plotting
    if args.std_table is not None or args.spectrum is not None:
        plt.figure()
        ...
        for ifile in list_files:
            ...
            for i in range(0, len(tbdata)):
                ...
                if bunit == "Jy":
                    ...
                    if args.efficiency is True:
                        eperjy = (
                            gtceffarea * (1.51e3 / lambda_i)
                        ) * cdelt  # Photons to be collected /GTC/sec/pix/Jy
    ...
    if args.catalog is not None:
        ...
        with open(args.catalog.name, "r") as csvfile:
            ...
            for i in a[:, 1]:
                if (
                    args.min_lambda is None or (float(i) > (args.min_lambda * (1 + z)))
                ) and (
                    args.max_lambda is None or (float(i) < (args.max_lambda * (1 + z)))
                ):
    ...
    if args.spectrum is not None:
        if bunit == "Jy" or "CGS" or "cgs" in bunit:
            ...
            if args.efficiency is True:
                ...
                plt.ylabel("ELECTRON")
    elif args.std_table is not None:
        plt.ylabel("flux [erg s$^{-1}$ cm$^{-2}$ A$^{-1}$]")
    ...
    if args.min_flambda is not None:
        plt.ylim(bottom=args.min_flambda)
    if args.max_flambda is not None:
        plt.ylim(top=args.max_flambda)

    if args.std_table is not None or args.spectrum is not None:
        plt.xlabel("wavelength")
        if args.no_legend is True:
