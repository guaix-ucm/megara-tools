...
def main(args=None):
    ...
    if args.spectrum is not None:
        ...
        with PdfPages(args.output) as pdf:
            ...
            EEWD = (
                []
            )  # Error of Flux from window_data - window_continuum / mean_continuum
            ...
            for ispec in range(1, nx + 1):
                ...
                if (
                    ...
                    and (np.abs(amp) / rms) >= float(args.limsnr)
                    and ispec != 623
                ):
                    FIB.append(ispec)
                ...
                if (
                    (np.isfinite(peak / rms))
                    and (np.abs(peak) / rms) >= float(args.limsnr)
                    and ispec != 623
                ):
        ...
        all_output[:, 5] = (
            EWD  # 5+1 Flux from window_data - window_continuum / mean_continuum
        )
        ...
        all_output[:, 9] = (
            H1  # 9+1 central lambda for methods 0 & 1 & 2 (first gaussian)
        )
        all_output[:, 10] = (
            H2  # 10+1 sigma (in AA) for methods 0 & 1 & 2 (first gaussian)
        )
        ...
        all_output[:, 18] = (
            H2KLC  # 18+1 sigma in km/s from H2 corrected for instrumental sigma (1st g)
        )
        all_output[:, 19] = H1KSB  # 19+1 velocity in km/s from H1B (2nd g)
        all_output[:, 20] = H2KSB  # 20+1 sigma in km/s from H2B (2nd g)
        all_output[:, 21] = (
            H2KLCB  # 21+1 sigma in km/s from H2 corrected for instrumental sigma (2nd g)
        )
        ...
        all_output[:, 24] = (
            EFLUXD  # 24+1 Error of 4 (Flux from window_data - window_continuum)
        )
        all_output[:, 25] = (
            EEWD  # 25+1 Error of 5 (Flux from window_data - window_continuum / mean_continuum)
        )
