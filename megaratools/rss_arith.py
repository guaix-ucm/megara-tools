#
# Copyright 2020-2024 Universidad Complutense de Madrid
#
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Combining by averaging aligned RSS files", prog="combine_rss"
    )
    parser.add_argument(
        "rss", help="Input table with list of RSS files", type=argparse.FileType("rb")
    )
    parser.add_argument(
        "-e",
        "--equation",
        metavar="Equation to evaluate",
        help="Example: '(ima1[:,9] + ima2[:,10])/ ima3[:,3]'",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="combined_rss.fits",
        metavar="OUTPUT RSS",
        help="Output RSS",
