...
def residuals(paramsin,x,y,z,ez,errors):
    ...
    vr = a * np.arctan(b*rr)
    zmodel = vsys + vr*np.sin(inclrad)*np.cos(theta)*np.sqrt((np.cos(inclrad)**2)/(1-np.sin(inclrad)**2*np.cos(theta)**2)) - z
    if (errors):
...
def vfunc(paramsin,x,y,z):
    ...
    vr = a * np.arctan(b*rr)
    zmodel = vsys + vr*np.sin(inclrad)*np.cos(theta)*np.sqrt((np.cos(inclrad)**2)/(1-np.sin(inclrad)**2*np.cos(theta)**2))
    return zmodel
...
def main(args=None):
    ...
    parser.add_argument('-g', '--title-color', metavar='TITLE/GRID COLOR', default='k', help='Title & grid color (default=k=key)')
    parser.add_argument('-A', '--alpha', metavar='ALPHA', default=0.0, help='Alpha value for grid (def=0=transp; 1=solid black)', type=float)
    parser.add_argument('-O', '--output-plot', metavar='OUTPUT PLOT', help='Plot output file (PDF)')
    parser.add_argument('-a', '--a-par', metavar='PARAMETER A', help='Parameter a (km/s)', type=float)
    ...
    barycorr = SkyCoord("01:58:00 +65:43:05", frame='fk5', unit=(u.hourangle, u.deg)).radial_velocity_correction('barycentric', obstime=Time('2019-09-24T02:23:22.19'), location=gtc)
...
# Plotting the input velocity map
    ...
    ax.coords.grid(color=args.title_color, alpha=args.alpha, linestyle='dashed')
    plt.title('Observed radial velocity', x=0.04, y=0.9, zorder=30, loc='left', color=args.title_color)
...
# Plotting the initial model
    ...
    col = hexplot(ax, x, y, imodel, scale=SCALE*args.rescale, cmap=args.palette, vmin=args.zcut1, vmax=args.zcut2, alpha=0.99, zorder=20)
    cb = plt.colorbar(col)
    cb.set_label(args.units, fontsize=19)
    ax.coords.grid(color=args.title_color, alpha=args.alpha, linestyle='dashed')
    plt.title('Radial velocity initial model', x=0.04, y=0.9, zorder=30, loc='left', color=args.title_color)
    ax.coords['dec'].set_axislabel('DEC (J2000)')
    ax.coords['ra'].set_axislabel('RA (J2000)')
    if args.output_plot is None:
        plt.show()
...
# Fitting ...
    verr = lambda p,x,y,z,ez: residuals(p,x,y,z,ez,args.errors)
    mini = lmfit.Minimizer(verr, p_v, fcn_args=(xspx,yspx,zspx,ezspx), nan_policy='omit')
...
# Plot confidence intervals ("a vs incldg" and "a vs b")
    if (args.confidence_intervals):
        ...
        propertyx = {'(0, 0)': 'a', '(0, 1)': 'b', '(0, 2)':  'pa', '(0, 3)':  vsys, '(0, 4)':  xcnt, '(0, 5)':  ycnt, '(0, 6)': incl,
                     '(1, 0)': 'a', '(1, 1)': 'b', '(1, 2)':  'pa', '(1, 3)':  vsys, '(1, 4)':  xcnt, '(1, 5)':  ycnt, '(1, 6)': incl,
                     '(2, 0)': 'a', '(2, 1)': 'b', '(2, 2)':  'pa', '(2, 3)':  vsys, '(2, 4)':  xcnt, '(2, 5)':  ycnt, '(2, 6)': incl,
                     '(3, 0)': 'a', '(3, 1)': 'b', '(3, 2)':  'pa', '(3, 3)':  vsys, '(3, 4)':  xcnt, '(3, 5)':  ycnt, '(3, 6)': incl,
                     '(4, 0)': 'a', '(4, 1)': 'b', '(4, 2)':  'pa', '(4, 3)':  vsys, '(4, 4)':  xcnt, '(4, 5)':  ycnt, '(4, 6)': incl,
                     '(5, 0)': 'a', '(5, 1)': 'b', '(5, 2)':  'pa', '(5, 3)':  vsys, '(5, 4)':  xcnt, '(5, 5)':  ycnt, '(5, 6)': incl,
                     '(6, 0)': 'a', '(6, 1)': 'b', '(6, 2)':  'pa', '(6, 3)':  vsys, '(6, 4)':  xcnt, '(6, 5)':  ycnt, '(6, 6)': incl}

        labelsx   = {'(0, 0)': 'a (km/s)', '(0, 1)': 'b (arcsec^-1)', '(0, 2)':  'pa (dg)', '(0, 3)':  vsysl, '(0, 4)':  xcntl, '(0, 5)':  ycntl, '(0, 6)': incll,
                     '(1, 0)': 'a (km/s)', '(1, 1)': 'b (arcsec^-1)', '(1, 2)':  'pa (dg)', '(1, 3)':  vsysl, '(1, 4)':  xcntl, '(1, 5)':  ycntl, '(1, 6)': incll,
                     '(2, 0)': 'a (km/s)', '(2, 1)': 'b (arcsec^-1)', '(2, 2)':  'pa (dg)', '(2, 3)':  vsysl, '(2, 4)':  xcntl, '(2, 5)':  ycntl, '(2, 6)': incll,
                     '(3, 0)': 'a (km/s)', '(3, 1)': 'b (arcsec^-1)', '(3, 2)':  'pa (dg)', '(3, 3)':  vsysl, '(3, 4)':  xcntl, '(3, 5)':  ycntl, '(3, 6)': incll,
                     '(4, 0)': 'a (km/s)', '(4, 1)': 'b (arcsec^-1)', '(4, 2)':  'pa (dg)', '(4, 3)':  vsysl, '(4, 4)':  xcntl, '(4, 5)':  ycntl, '(4, 6)': incll,
                     '(5, 0)': 'a (km/s)', '(5, 1)': 'b (arcsec^-1)', '(5, 2)':  'pa (dg)', '(5, 3)':  vsysl, '(5, 4)':  xcntl, '(5, 5)':  ycntl, '(5, 6)': incll,
                     '(6, 0)': 'a (km/s)', '(6, 1)': 'b (arcsec^-1)', '(6, 2)':  'pa (dg)', '(6, 3)':  vsysl, '(6, 4)':  xcntl, '(6, 5)':  ycntl, '(6, 6)': incll}

        propertyy = {'(0, 0)': 'a',  '(0, 1)': 'a',  '(0, 2)':  'a', '(0, 3)':  'a', '(0, 4)': 'a',  '(0, 5)': 'a',  '(0, 6)':  'a',
                     '(1, 0)': 'b',  '(1, 1)': 'b',  '(1, 2)':  'b', '(1, 3)':  'b', '(1, 4)': 'b',  '(1, 5)': 'b',  '(1, 6)':  'b',
                     '(2, 0)': 'pa', '(2, 1)': 'pa', '(2, 2)': 'pa', '(2, 3)': 'pa', '(2, 4)': 'pa', '(2, 5)': 'pa', '(2, 6)': 'pa',
                     '(3, 0)': vsys, '(3, 1)': vsys, '(3, 2)': vsys, '(3, 3)': vsys, '(3, 4)': vsys, '(3, 5)': vsys, '(3, 6)': vsys,
                     '(4, 0)': xcnt, '(4, 1)': xcnt, '(4, 2)': xcnt, '(4, 3)': xcnt, '(4, 4)': xcnt, '(4, 5)': xcnt, '(4, 6)': xcnt,
                     '(5, 0)': ycnt, '(5, 1)': ycnt, '(5, 2)': ycnt, '(5, 3)': ycnt, '(5, 4)': ycnt, '(5, 5)': ycnt, '(5, 6)': ycnt,
                     '(6, 0)': incl, '(6, 1)': incl, '(6, 2)': incl, '(6, 3)': incl, '(6, 4)': incl, '(6, 5)': incl, '(6, 6)': incl}

        labelsy   = {'(0, 0)': 'a (km/s)',  '(0, 1)': 'a (km/s)',  '(0, 2)':  'a (km/s)', '(0, 3)':  'a (km/s)', '(0, 4)': 'a (km/s)',  '(0, 5)': 'a (km/s)',  '(0, 6)':  'a (km/s)',
                     '(1, 0)': 'b (arcsec^-1)',  '(1, 1)': 'b (arcsec^-1)',  '(1, 2)':  'b (arcsec^-1)', '(1, 3)':  'b (arcsec^-1)', '(1, 4)': 'b (arcsec^-1)',  '(1, 5)': 'b (arcsec^-1)',  '(1, 6)':  'b (arcsec^-1)',
                     '(2, 0)': 'pa (dg)', '(2, 1)': 'pa (dg)', '(2, 2)': 'pa (dg)', '(2, 3)': 'pa (dg)', '(2, 4)': 'pa (dg)', '(2, 5)': 'pa (dg)', '(2, 6)': 'pa (dg)',
                     '(3, 0)': vsysl, '(3, 1)': vsysl, '(3, 2)': vsysl, '(3, 3)': vsysl, '(3, 4)': vsysl, '(3, 5)': vsysl, '(3, 6)': vsysl,
                     '(4, 0)': xcntl, '(4, 1)': xcntl, '(4, 2)': xcntl, '(4, 3)': xcntl, '(4, 4)': xcntl, '(4, 5)': xcntl, '(4, 6)': xcntl,
                     '(5, 0)': ycntl, '(5, 1)': ycntl, '(5, 2)': ycntl, '(5, 3)': ycntl, '(5, 4)': ycntl, '(5, 5)': ycntl, '(5, 6)': ycntl,
                     '(6, 0)': incll, '(6, 1)': incll, '(6, 2)': incll, '(6, 3)': incll, '(6, 4)': incll, '(6, 5)': incll, '(6, 6)': incll}
        ...
        for row in range(0, rows+1):
            for col in range(0, row):
                ...
                cx, cy, grid = lmfit.conf_interval2d(mini, fitout_v, propertyx[key], propertyy[key], 30, 30)
...
# Plot velocity model only in valid pixels/spaxels
    ...
    col = hexplot(ax, xspx, yspx, bzmodel, scale=SCALE*args.rescale, cmap=args.palette, vmin=args.zcut1, vmax=args.zcut2, alpha=0.99, zorder=20)
    cb = plt.colorbar(col)
    cb.set_label(args.units, fontsize=19)
    ax.coords.grid(color=args.title_color, alpha=args.alpha, linestyle='dashed') 
    plt.title('Radial velocity model', x=0.04, y=0.9, zorder=30, loc='left', color=args.title_color)
    ax.coords['dec'].set_axislabel('DEC (J2000)')
    ax.coords['ra'].set_axislabel('RA (J2000)')
    if args.output_plot is None:
        plt.show()
...
# Plot velocity residuals only in valid pixels/spaxels
    ...
    ax.set_xlim([-6.5, 6.5])
    ax.set_ylim([-6.5, 6.5])
    col = hexplot(ax, xspx, yspx, bnnres, scale=SCALE*args.rescale, cmap=args.palette, vmin=-30., vmax=30., alpha=0.99, zorder=20)
    cb = plt.colorbar(col)
    cb.set_label(args.units, fontsize=19)
    ax.coords.grid(color=args.title_color, alpha=args.alpha, linestyle='dashed') 
    plt.title('Radial velocity residuals', x=0.04, y=0.9, zorder=30, loc='left', color=args.title_color)
    ax.coords['dec'].set_axislabel('DEC (J2000)')
    ax.coords['ra'].set_axislabel('RA (J2000)')
    if args.output_plot is None:
        plt.show()
...
# Plot entire velocity model
    ...
    bnnres = (z - bzmodel)
    bzres = (z - bzmodel)/ez
    col = hexplot(ax, x, y, bzmodel, scale=SCALE*args.rescale, cmap=args.palette, vmin=args.zcut1, vmax=args.zcut2, alpha=0.99, zorder=20)
    cb = plt.colorbar(col)
    cb.set_label(args.units, fontsize=19)
    ax.coords.grid(color=args.title_color, alpha=args.alpha, linestyle='dashed')
    plt.title('Radial velocity model', x=0.04, y=0.9, zorder=30, loc='left', color=args.title_color)
